...
def create_or_refresh_materialized_view(
    ...
    try:
        ...
        query_job.result()
        logging.info(
            f"Materialized view {project_id}.{dataset_id}.{view_id} created or refreshed successfully."
        )

    except GoogleCloudError as e:
        logging.error(f"Error creating or refreshing the materialized view: {e}")
        raise
