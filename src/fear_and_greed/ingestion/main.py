...
def ingest_fear_greed_raw():
    ...
    raw_fear_greed_data = bigquery_raw_data_table(
        bigquery_client=bigquery_client, dataset_id='shabubsinc_db', table_id='raw_daily_fear_greed_data', api_data=fear_greed_data)

    try:
        stream_data_to_bigquery(bigquery_client=bigquery_client, data=raw_fear_greed_data,
                                project_id='shabubsinc', dataset_id='shabubsinc_db', table_id='raw_daily_fear_greed_data')
        publisher.publish(
            topic_path, b"Trigger clean processing for Fear & Greed")
