...
import logging
from shared_functions.datetime_helper import convert_unix_timestamp_in_data, convert_datetime_to_string

def filter_duplicates_ohlc(bigquery_client: bigquery.Client, dataset_id: str, table_id: str, raw_data: List[Dict]) -> List[Dict]:
    """
    ...
    existing_data_modified = {row['time_period_start'] for row in results}  # A set of existing timestamps
    ...
    for row in raw_data:
        ...
        if data_modified:
            try:
                ...
                formatted_date = datetime.strptime(data_modified[:-4] + 'Z', '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%Y-%m-%d %H:%M:%S UTC')
            except ValueError:
...
def filter_duplicates_fear_greed(bigquery_client: bigquery.Client, dataset_id: str, table_id: str, raw_data: List[Dict]) -> List[Dict]:
    ...
    existing_data_modified = {row['timestamp'] for row in results}  # A set of existing timestamps
    
    existing_data_modified = convert_datetime_to_string(existing_data_modified)
    new_data = [
    row for row in raw_data
    if row.get('timestamp') not in existing_data_modified]
